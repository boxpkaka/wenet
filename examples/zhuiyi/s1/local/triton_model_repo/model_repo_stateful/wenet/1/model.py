...
from ctc_decoder import PathTrie, TrieVector
...
class TritonPythonModel:
  ...
  def execute(self, requests):
    ...
    for request in requests:
      ...
      if ready:
        if corrid not in self.seq_states.keys():
          print(f"ERROR: {corrid} NOT INIT")
        root, encoder_out = self.seq_states[corrid]
        trie_vector.append(root)
    ...
    batch_states = [
        trie_vector, batch_start, batch_end, batch_encoder_hist, cur_encoder_out
    ]
    res_sents, res_scores, new_states = self.model.infer(
        batch_log_probs, batch_log_probs_idx, batch_len, rescore_index,
        batch_states)
    cur_encoder_out = new_states
    for i in range(len(res_sents)):
      sent = np.array(res_sents[i])
      score = np.array(res_scores[i])
      out_tensor_0 = pb_utils.Tensor("OUTPUT0", sent.astype(self.output0_dtype))
      out_tensor_1 = pb_utils.Tensor("OUTPUT1",
                                     score.astype(self.output1_dtype))
      ...
      if i in rescore_index:
        ...
        if self.model.rescoring:
          if self.seq_states[corr][1] is None:
            ...
            new_hist = torch.cat([self.seq_states[corr][1], cur_encoder_out[i]],
                                 axis=0)
