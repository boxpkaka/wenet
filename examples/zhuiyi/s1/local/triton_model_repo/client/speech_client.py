...
class StreamingSpeechClient(object):
  ...
  def __init__(self, triton_client, model_name, protocol_client, args):
    ...
    self.test_performence = args.test_performence
    self.silence_percent = args.silence_percent
  ...
  def recognize(self, wav_file, idx=0):
    ...
    if self.test_performence:
      sli_time = len(waveform) / sample_rate * self.silence_percent / (
          1 - self.silence_percent)
      time.sleep(sli_time)
    ...
    for idx, seg in enumerate(wav_segs):
      ...
      if self.test_performence:
        end_time = time.time()
        compute_cost = math.ceil((end_time - start_time) * 1000)
